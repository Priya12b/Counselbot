...
//   /* fetch lawyers + clients on mount */
//   useEffect(() => {
...
//       .catch(err => toast.error(err.response?.data?.error || "Error assigning"));
//   };

//   /* simple styles */
//   const box = { background:"#fff", padding:"1rem", borderRadius:"12px",
//                 boxShadow:"0 4px 12px rgba(0,0,0,0.1)", maxWidth:"400px",
//                 margin:"0 auto" };

//   return (
//     <div style={box}>
//       <h3>👑 Assign Client to Lawyer</h3>

//       <label>Client:</label><br/>
//       <select value={clientId} onChange={e=>setClientId(e.target.value)} style={{width:"100%",marginBottom:"1rem"}}>
//         <option value="">-- select client --</option>
//         {clients.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
//       </select>

//       <label>Lawyer:</label><br/>
//       <select value={lawyerId} onChange={e=>setLawyerId(e.target.value)} style={{width:"100%",marginBottom:"1rem"}}>
//         <option value="">-- select lawyer --</option>
//         {lawyers.map(l => <option key={l.id} value={l.id}>{l.name}</option>)}
//       </select>

//       <button onClick={assign} style={{padding:"8px 16px",background:"#4a90e2",
